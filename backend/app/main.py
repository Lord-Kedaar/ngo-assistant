...
async def chat(p:ChatIn,request:Request,response:Response):
 ...
 async with sem:
  ...
  if settings.generative_mode:
   context='\n\n'.join('Źródło: '+c['filename']+' — '+c['section_title']+'\n'+(' '.join(c['text'].split())[:1200]) for _,c in hits)
   messages=[
   ...
   raw=await complete(messages); raw=re.sub(r'\s*\[Źródło:[^\]]+\]\s*',' ',raw).strip(); ans=sanitize_markdown(raw + '\n\n['+txt.get('source_label','Źródło')+': '+best_chunk['filename']+' — '+best_chunk['section_title']+']')
  else:
